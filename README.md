...
<br/>
Kotlin + Spring Boot 기반 **나무위키 데이터셋** 적재 서비스입니다. Hugging Face의 `heegyu/namuwiki` 데이터를 받아 임베딩을 생성한 뒤 PostgreSQL **pgvector**에 저장하며, Thymeleaf 대시보드에서 실시간 진행 상황을 확인할 수 있습니다.
...
| **기술 스택** | Kotlin 1.9, Spring Boot 3.3, Thymeleaf, JDBC, OkHttp, Parquet, Chart.js, jQuery |
...
│   │   ├── kotlin/
│   │   │   └── sleekydz86/namuwiki/
│   │   │       ├── NamuwikiIngestionApplication.kt    # Spring Boot 엔트리포인트
│   │   │       │
│   │   │       ├── config/                            # 설정
│   │   │       │   ├── AsyncConfig.kt                 # @EnableAsync
│   │   │       │   ├── EmbeddingConfig.kt             # namuwiki.embedding
│   │   │       │   └── NamuwikiIngestionConfig.kt     # DatasetConfig, InsertConfig, @EnableConfigurationProperties
│   │   │       │
│   │   │       ├── domain/                            # 도메인
│   │   │       │   ├── NamuwikiDoc.kt                 # 적재 문서 (title, content, embedding, namespace, contributors)
│   │   │       │   ├── NamuwikiRow.kt                 # 원본 행 (title, text, namespace, contributors)
│   │   │       │   └── port/
│   │   │       │       └── EmbeddingClient.kt        # 임베딩 배치 연산 인터페이스
│   │   │       │
│   │   │       ├── application/                      # 애플리케이션 서비스
│   │   │       │   ├── IngestionProgress.kt           # ProgressSnapshot, IngestionStatus, IngestionProgressState
│   │   │       │   ├── IngestionProgressHolder.kt     # 진행 상태 보관·갱신 (스레드 세이프)
│   │   │       │   ├── IngestionRunner.kt             # @Async ingestion 트리거
│   │   │       │   └── NamuwikiIngestionService.kt   # 파이프라인: HF → 임베딩 → pgvector
│   │   │       │
│   │   │       ├── presentation/                     # HTTP·뷰
│   │   │       │   ├── DashboardController.kt         # GET / → Thymeleaf 대시보드
│   │   │       │   └── IngestionController.kt        # POST /api/ingest, GET /api/progress, GET /api/stats
│   │   │       │
│   │   │       └── infrastructure/                    # 인프라
│   │   │           ├── huggingface/
│   │   │           │   └── HfDatasetService.kt       # HF parquet URL 조회, 다운로드, 행 스트리밍
│   │   │           ├── embedding/
│   │   │           │   └── HttpEmbeddingClient.kt    # EmbeddingClient 구현 (HTTP POST)
│   │   │           └── persistence/
│   │   │               └── NamuwikiDocRepository.kt   # pgvector 배치 INSERT, count
│   │   │
│   │   └── resources/
│   │       ├── application.yml
│   │       ├── schema.sql                             # vector 확장, namuwiki_doc 테이블·인덱스
│   │       └── templates/
│   │           └── dashboard.html                      # Thymeleaf 실시간 대시보드 (Chart.js, jQuery)
│   │
│   └── test/
│       └── kotlin/
└── README.md
...
| 패키지 | 역할 |
|--------|------|
| `sleekydz86.namuwiki` | 루트·엔트리 |
| `config` | Spring 설정·ConfigurationProperties (embedding, dataset, insert, async) |
...
| `infrastructure.embedding` | 외부 임베딩 HTTP 클라이언트 |
...
- Java 21+
- PostgreSQL 17 + pgvector (예: `postgresql/docker-compose.pgvector.yml`로 5433 포트 실행)
- 임베딩 HTTP API (아래 규격)
...
`src/main/resources/schema.sql`을 DB에 한 번 실행하세요. 벡터 차원(384)은 사용하는 임베딩 모델에 맞게 수정해야 합니다.

```bash
cd postgresql
docker-compose -f docker-compose.pgvector.yml up -d pgvector
# pgAdmin 또는 psql로 127.0.0.1:5433 접속 후
\i schema.sql
```
...
| 설정 | 환경 변수 | 설명 |
|------|-----------|------|
| DB URL | `SPRING_DATASOURCE_URL` | 기본: `jdbc:postgresql://127.0.0.1:5433/postgres` |
| 임베딩 API URL | `EMBEDDING_ENDPOINT_URL` | **필수**. POST로 텍스트 배열 받아 임베딩 배열 반환 |
| 임베딩 API 키 | `EMBEDDING_API_KEY` | 선택 |
...
- 요청: `POST {EMBEDDING_ENDPOINT_URL}`  
  - Body: `{ "texts": ["문장1", "문장2", ...] }`  
  - `Content-Type: application/json`
- 응답: `{ "embeddings": [[f1,f2,...], ...] }`  
  - `texts` 순서와 동일한 개수의 float 배열, 차원은 DB `vector(n)`과 일치(예: 384)
